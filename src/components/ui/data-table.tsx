...
// Retro-Playful Data Table: sortable, clean table with Linear styling
...
function DataTable<T extends Record<string, DataTableCell>>({
  ...
  return (
    <div className={cn("w-full overflow-auto rounded-lg border-2 border-border shadow-[var(--shadow-hard)]", className)}>
      <table className="w-full caption-bottom text-sm">
        ...
        <tbody>
          {sortedData.map((row, i) => (
            <tr
              key={i}
              className="border-b-2 border-border/50 bg-background transition-colors last:border-0 hover:bg-secondary/30"
            >
              {columns.map((col) => (
                <td
                  key={String(col.key)}
