...
// Retro-Playful Sidebar: navigation sidebar component
// Clean left nav with bg-background, border-r-2 border-border
...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  ({ className, collapsed, children, ...props }, ref) => (
    <div
      ...
      className={cn(
        "flex h-full flex-col border-r-2 border-border bg-background",
...
const SidebarHeader = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("flex h-14 items-center border-b-2 border-border px-4", className)}
      {...props}
    />
  )
)
...
const SidebarFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("border-t-2 border-border p-2", className)}
      {...props}
    />
  )
)
