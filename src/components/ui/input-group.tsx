...
// Retro-Playful InputGroup: input with prefix/suffix addons, visually merged inside border-2
// Addons share the same rounded-lg container as the input
    ...
    return (
      ...
      >
        ...
        {prefix && (
          <div
            className={cn(
              "flex items-center justify-center border-r-2 border-border bg-secondary px-3",
        ...
        {suffix && (
          <div
            className={cn(
              "flex items-center justify-center border-l-2 border-border bg-secondary px-3",
              "text-sm text-muted-foreground select-none shrink-0",
              "[&>svg]:h-4 [&>svg]:w-4",
              size === "default" ? "h-10" : "h-9"
            )}
